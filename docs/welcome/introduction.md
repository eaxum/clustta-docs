...
Welcome to Clustta — open-source version control, collaboration and asset management built specifically for creative work.

Clustta is for the people who make things: animators, designers, illustrators, video editors, 3D artists, game developers and the studios they work in. It gives teams a single place to organize their projects, version their files, hand work off to collaborators, and keep production moving — without the friction of tools designed for software engineers or the fragility of consumer file sync.
...
5. Studios run their own server, use a managed cloud instance, or stay fully offline in **Personal** mode. Your call.
...
| Linux (Ubuntu/Debian-based) | ✅ Beta |
| Studio Server | ✅ Docker (any Linux host) |

## Where to go next

- **New here?** → Read the [Philosophy](./philosophy.md) to understand how Clustta thinks.
- **Want to dive in?** → [Clustta 101](./clustta-101.md) is a 5-minute crash course.
- **Setting up a team?** → Jump to [Getting Started](../getting-started/install.md).
- **Want the technical picture?** → See the [Architecture overview](../architecture/overview.md).
