...
- **Personal studio** — Always available, fully offline. Perfect for solo work or trying things out. Everything stays on your machine.
- **Online studio** — A team studio with a server (self-hosted or Clustta Cloud). Adds collaborators, roles, sharing, and sync.
...
- **Personal:** Click **New Project**, give it a name, pick a working folder. Done.
...
- **Use an Asset Template** from inside Clustta. Clustta creates the file for you using a template the studio has set up (e.g. a base `.blend` with the studio's render settings).
...
### 4. (Online studios only) Sync

Checkpoints are local until you sync. When you press **Sync** (or `Ctrl+S`), Clustta uploads your new checkpoints to the studio server. Collaborators can then pull them down.
...
- **Selective sync.** You only download the assets assigned to you (plus anything in collections marked as **Library**, which everyone can see). You're not dragging down 500 GB of project history just to fix one shot.
